...
/* Envia pedaços com TAMANHO números para cada processo */
    if (meu_ranque == 0) { 
        ...
        for (dest; dest < num_procs; dest++){
            MPI_Isend(&inicio, 1, MPI_INT, dest, 99, MPI_COMM_WORLD, &pedido_envia);
            count_aux++;
        }


/* Fica recebendo as contagens parciais de cada processo */
        while (stop < (num_procs-1-count_aux)) {
...
/* Cada processo escravo recebe o início do espaço de busca */
        while (estado.MPI_TAG != 99) {
            MPI_Irecv(&inicio, 1, MPI_INT, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &pedido_recebe);
            MPI_Wait(&pedido_recebe, &estado);
